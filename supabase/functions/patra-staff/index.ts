    ...
    try {
        ...
        if (req.method === 'POST' && (url.pathname.includes('/staff') || url.pathname.includes('/create-card'))) {
            ...
            const { 
                ...
                job_title, 
                staff_id, 
                username, // New: accept username as alternate for staff_id
                designation, 
                phone,
                metadata = {} 
            } = body;
            ...
            // Generate a staff ID if not provided, preferring username if available
            // This ensures predictable URLs like /company/username
            const finalStaffId = staff_id || username || designation || `STF-${Math.random().toString(36).substring(2, 8).toUpperCase()}`;
            ...
            const { data: employee, error: insertError } = await supabase
                ...
                .insert({
                    ...
                    status: 'approved',
                    is_approved: true, // Auto-approve to make card live immediately
            ...
            // Construct Card Response - ensure we use the testing domain or base URL
            // Base URL normally comes from the req origin, but let's make it robust
            const origin = req.headers.get('origin') || 'https://vaw-patra.vercel.app';
            const cardUrl = `${origin}/${company.vanity_url}/${employee.staff_id}`;
